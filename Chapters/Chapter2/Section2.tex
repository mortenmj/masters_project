...
Typically, Modelica models are translated into a flat system of differential and algebraic equations (DAEs) \cite{lundvall2008}.
...
Solving such a systems requires that equations are differentiated, as we will see in the following example which is adapted from \cite{claus2002}.
...
The final representation of the system consists of both the continuous-time and discrete-time behavior of the system\cite{lundvall2008}:
\loadeq{\rootfolder/Equations.tex}{008}

The continuous-time behavior, between events, is given by \cref{eq:dae-continuous}.
The equation warrants some explanation.

\begin{itemize}[noitemsep]
    \item $\vec{x}(t)$ are the dynamic variables of the system
    \item $\vec{u}(t)$ are the system inputs
    \item $\vec{y}(t)$ are other variables including outputs
    ...
    \item $\vec{p}$ is a vetor of system parameters and constants.
...
The equations are more thoroughly explained in \cite{lundvall2008}.
...
\begin{figure}[H]
    ...
    \caption{Solution algorithm flowchart}
    \label{fig:dae-flowchart}
\end{figure}

As stated, the system description in \cref{eq:dae-continuous} is only valid between events, and the simulation is therefore performed piecewise.
...
A flowchart describing the procedure is shown in \cite{lundvall2008}, and reproduced in \cref{fig:dae-flowchart}.
...
\begin{enumerate}
	...
	Once any of the conditions in $\vec{c}$ which are relevant (given by $\vec{rel}(\vec{v}(t_{e}))$), the continuous simulation is halted and event conditions are evaluated.
