...
As Modelica is object-oriented it is possible, if desired, to separate a model into smaller component parts, and connect these components to one another to form a more complex model.
...
Looking at the code of the first model, shown in \cref{fig:modelica-inertial}, it can be seen that it does exactly what is proposed above.
\Cref{lst:modelica-inertial-basic} shows the the code generated when the model in \cref{fig:modelica-inertial} was created, although somewhat cleaned up to make the example clearer.
...
In \cref{fig:modelica-inertial-expanded} actuator has been added, as well as a sensor for measuring the rotation speed of the second shaft.
...
\begin{figure}[ht]
    ...
    \caption{Expanded view of the actuator left and process subsystems.\label{fig:modelica-inertial-architecture-blowup}}
\end{figure}
...
Contrary to the example above, it is necessary to derive the equations for the circuit and then implement it using blocks that implement basic mathematical operations.
...
above model could easily be modified by replacing a component with any other two-port component.
...
In this case, the "ground" is represented by a stepping line, which starts one meter below the initial height for the ball.
...
Solving such a systems requires that equations are differentiated as shown in following example, in \cref{fig:modelica-inertial-equations}.
...
\begin{wraptable}{R}{0.5\columnwidth}
    ...
    \captionof{listing}{Simulating connected FMUs.\label{lst:connected_fmus}}
\end{wraptable}
...
In the example, the \emph{scipy.signal.square()} function, which generates a square waveform, is attached to the input of the system from \cref{fig:modelica-inertial-architecture}.
